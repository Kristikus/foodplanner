      ...
      return getData(url)
        .then((data) => {
          ...
          if (!data || !data.results || data.results.length === 0) {
            console.error('No data or recipes found:', data); // Vérifier les données reçues //
            cardContainer.innerHTML =
              '<p>No recipes found. Please try again later.</p>'; // Afficher un message d'erreur //
            return;
          }
        ...
        .catch((error) => {
          console.error('Error fetching data:', error);
          cardContainer.innerHTML =
            '<p>Failed to fetch recipes. Please try again later.</p>'; // Afficher un message d'erreur //
        });
    }
